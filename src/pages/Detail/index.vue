        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            ...
            <div class="cartWrap">
              ...
              <div class="add">
                <!-- 以前的路由跳转，从A路由跳转到B路由，这里在加入购物车进行路由跳转之前，
...
export default {
  ...
  methods: {
    ...
    async addShopCar() {
      //发请求--将产品加入到数据库（通知服务器）
      //在点击加入购物车这个按钮的时候，做的第一件事将参数带给服务器（发请求），通知服务器加入购物车的产品是什么
      try {
        // this.$store.dispatch其实就是调用了一个函数
        await this.$store.dispatch("addOrUpdateShopCart", {
          skuId: this.$route.params.skuid,
          skuNum: this.skuNum,
        });
        //路由跳转
        //一些简单数据通过query形式给路由组件传递过去
        //复杂数据通过会话存储（不持久化，关闭即消失）
        //本地存储和会话存储：一般存储的是字符串
